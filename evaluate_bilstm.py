...
def load_cleaned_frame() -> pd.DataFrame:
    """Prefer frozen training CSV; else rebuild from brand_task (same as bilstm_train)."""
    frozen = ARTIFACT_DIR / "cleaned_training_data.csv"
    if frozen.is_file():
        return pd.read_csv(frozen, encoding="utf-8-sig")
    print("cleaned_training_data.csv missing — run bilstm_train.py first or use DATA_PATH rebuild.")
...
def main() -> None:
    ...
    # Train set (sanity / overfitting check)
    ...
    print(f"(Large gap vs val may indicate overfitting.)")

    print("\n--- Per-class report (validation, first 25 classes by support) ---")
    # Map indices to ADG codes for readability
    ...
    # Truncate long output: show summary lines only + full report to file
    lines = report.split("\n")
    print("\n".join(lines[:28]))
    if len(lines) > 28:
        print("  ... (truncated)")

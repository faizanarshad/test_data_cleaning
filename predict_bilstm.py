"""
Predict ADG_CODE with the saved BiLSTM model.
...
from bilstm_train import clean_text
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "-n",
        type=str,
        help="Product name (GOOD_NAME)",
    )
    parser.add_argument("--brand", "-b", type=str, default="", help="Brand")
    parser.add_argument("--category", "-c", type=str, default="", help="Category")
    parser.add_argument(
        "--text",
        "-t",
        type=str,
        help="Full text_input as used in training (overrides name/brand/category)",
    )
    parser.add_argument(
        "--top-k",
        type=int,
        default=5,
        help="Number of top ADG codes to show",
    )
